  ...
  <div class="mt-4 min-h-screen bg-white pb-28" style="color:#2a2a2a;">
    ...
    <div class="mx-auto w-full max-w-md px-4">
      ...
      <transition name="toast-pop">
        <div v-if="toastMessage"
          ...
          <span
            class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#3082e3] text-white toast-check">
      ...
      <template v-else>
        ...
        <div class="relative mt-5">
          ...
          <input v-model="filter" @input="onSearchInput" placeholder="Buscar por nombre o apellido..."
        ...
        <section class="mt-6">
          <div class="flex items-center justify-between">
            ...
            <span v-if="trustedContacts.length"
          ...
          <!-- Estado vacío -->
          <div v-if="!trustedContacts.length"
          ...
          <TransitionGroup v-else tag="div" name="contact-list" class="mt-3 space-y-2.5">
            <div v-for="c in trustedContacts" :key="c.id"
              ...
              <button @click="askRemoveContact(c)" :disabled="removingId === c.trusted_contact_id"
        ...
        <section class="mt-6">
          <button type="button" @click="addSectionOpen = !addSectionOpen"
            ...
            <div class="flex items-center gap-3">
              <span
                class="flex h-9 w-9 items-center justify-center rounded-full bg-white text-[#3082e3] shadow-[0_4px_12px_rgba(48,130,227,0.16)]">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#3082e3] transition-transform duration-300"
              :class="addSectionOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor"
              stroke-width="2.2">
            ...
            <div v-if="addSectionOpen" class="overflow-hidden">
              <div class="mt-3">
                <div v-if="filteredUsers.length === 0"
                  class="rounded-2xl bg-[#f7f9fc] px-4 py-6 text-center text-sm text-slate-500">
                  No hay usuarios disponibles para agregar.
                </div>

                <TransitionGroup v-else tag="div" name="contact-list" class="max-h-80 space-y-2 overflow-y-auto pr-1">
                  <button v-for="u in filteredUsers" :key="u.id" type="button" @click="askAddContact(u)"
                    ...
                    <div
                      class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#f7f9fc] text-slate-500 font-bold">
                      {{ initials(u) }}
                    </div>
                    <div class="min-w-0 flex-1">
    ...
    <div v-if="pendingAdd" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
      <div class="w-full max-w-sm rounded-[26px] bg-white p-6 shadow-[0_22px_48px_rgba(15,23,42,0.22)] modal-pop">
        <div
          class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#eef4ff] text-[#3082e3] text-xl font-bold">
          {{ initials(pendingAdd) }}
        </div>
        ...
        <div class="mt-6 space-y-3">
          <button type="button" @click="confirmAddContact" :disabled="addingId === pendingAdd.id"
          ...
          <button type="button" @click="cancelAddContact" :disabled="addingId === pendingAdd.id"
    ...
    <div v-if="pendingRemove" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
      <div class="w-full max-w-sm rounded-[26px] bg-white p-6 shadow-[0_22px_48px_rgba(15,23,42,0.22)] modal-pop">
        ...
        <div class="mt-6 space-y-3">
          <button type="button" @click="confirmRemoveContact"
          ...
          <button type="button" @click="cancelRemoveContact" :disabled="removingId === pendingRemove.trusted_contact_id"
            class="w-full rounded-2xl bg-[#f7f9fc] py-3 text-sm font-semibold text-slate-600 transition active:scale-[0.98]">
            Cancelar
          </button>
        </div>
