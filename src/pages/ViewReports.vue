...
<template>
  <section class="min-h-screen overflow-x-hidden bg-white px-4 pt-6 pb-24">
    <div class="mx-auto w-full max-w-[390px]">
      ...
      <div class="flex items-center gap-3  px-4 py-3 border-b border-gray-100">
        ...
        <div class="min-w-0">
          ...
          <p class="text-xs" style="color:#6b7280;"> Consultá reclamos de la comunidad y sumate a los que también te
            preocupan.</p>
        </div>
      </div>

      <button type="button" @click="showFilterSheet = true" class="inline-flex w-fit items-center gap-2 rounded-full bg-[#E0E5EC] px-4 py-2.5 text-sm font-medium text-slate-700
      ...
      <ul class="mb-8 space-y-4">
        <ReportCard v-for="r in reports" :key="r.id" :report="r" :to="`/report/${r.id}`"
          :supporting="supportingId === r.id" :already-supported="supportedReportIds.includes(r.id)"
    ...
    <div v-if="showFilterSheet" class="fixed inset-0 z-40 bg-black/35 backdrop-blur-[1px]"
      @click="showFilterSheet = false"></div>
    ...
    <div v-if="showFilterSheet"
      ...
      <div class="mt-5">
        ...
        <div class="space-y-2">
          <label
            class="flex items-center gap-3 rounded-2xl bg-[#E0E5EC] px-3 py-3 text-sm text-slate-700 shadow-[0_6px_16px_rgba(148,163,184,0.18)]">
