<template>
    <div class="flex flex-col h-full bg-white font-['Inter']" style="color: #2a2a2a;">
      ...
      ══════════════════════════════════════════ -->
      <div v-if="!ubicacionUsuario" class="flex-1 flex flex-col overflow-y-auto">
        ...
        <div class="flex flex-col items-center pt-10 pb-6 px-6 gap-3">
          <div class="w-20 h-20 rounded-full flex items-center justify-center" style="background:#eff6ff;">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="#3082e3" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 3C12 3 4 6.5 4 12v5.5l8 2.5 8-2.5V12C20 6.5 12 3 12 3z"/>
        ...
        <div class="px-4 flex flex-col gap-3">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="w-full rounded-xl pl-9 pr-10 py-3 text-sm border focus:outline-none transition-all"
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="#6b7280" stroke-width="2.5">
          ...
          <ul v-if="sugerencias.length" class="border border-gray-100 rounded-xl overflow-hidden shadow-md">
            ...
            >
              ...
              <span class="text-xs leading-relaxed" style="color:#2a2a2a;">{{ s.display_name }}</span>
            </li>
          </ul>
          ...
          <!-- Error -->
          <div v-if="error" class="px-4 py-3 rounded-xl border text-sm" style="background:#fff1f0; border-color:#fca5a5; color:#dc2626;">
            {{ error }}
          </div>
        </div>
      </div>
      ...
      <div v-if="ubicacionUsuario" class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <!-- Sin resultados -->
          <div v-if="puntosEnRadio.length === 0" class="flex flex-col items-center justify-center py-12 px-8 text-center gap-3">
            <div class="w-14 h-14 rounded-full flex items-center justify-center" style="background:#f3f4f6;">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="#9ca3af" stroke-width="1.5">
          ...
          <!-- Items -->
          <div
            v-for="(punto, index) in puntosEnRadio"
            :key="punto.id"
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                ...
                <span
                  v-if="puntoSeleccionado?.id === punto.id"
      ...
      <!-- ── Spinner global ── -->
      <div
        v-if="cargandoBusqueda || cargandoGPS"
  ...
  async function actualizarMapa() {
    ...
    // Ruta al punto seleccionado si existe
  ...
  async function trazarRuta(punto) {
    ...
    // Importar LRM — soporta distintas formas de export
    const lrmMod = await import('leaflet-routing-machine');
    const LRM    = lrmMod.default ?? lrmMod;
    // LRM extiende el objeto L global; asegurar que esté disponible
    ...
    rutaControl = Routing.control({
      ...
      createMarker: () => null,   // no crear markers propios de LRM
      router: Routing.osrmv1({
        serviceUrl: 'https://router.project-osrm.org/route/v1',
      }),
    }).addTo(mapa);
  
    // Ocultar el panel HTML que genera LRM si aparece
  ...
  watch(radioBusqueda, async () => {
    if (!ubicacionUsuario.value) return;
    // Deseleccionar si el punto seleccionado queda fuera del nuevo radio
    if (puntoSeleccionado.value) {
      const d = haversine(
