...
<template>
  <div class="min-h-screen bg-white pb-28 overflow-x-hidden" style="color:#2a2a2a;">
    <div class="mx-auto w-full max-w-md">
      ...
      <div class="relative w-full overflow-hidden">
        ...
        <div v-if="viewState === 'setup'" class="absolute top-3 left-3 right-3 z-20">
          <div class="relative">
            ...
            <input v-model="destinationQuery" @input="onDestinationInput" placeholder="¿A dónde querés ir?"
          ...
          <ul v-if="destinationSuggestions.length"
            class="mt-2 bg-white border border-gray-100 rounded-xl overflow-hidden shadow-[0_10px_24px_rgba(15,23,42,0.14)]">
            <li v-for="s in destinationSuggestions" :key="s.place_id" @click="pickDestination(s)"
      ...
      <section v-if="viewState === 'setup'" class="px-4 pt-5">
        <div class="rounded-[22px] border border-[#d6e8fb] bg-[#eef4ff] p-5">
          <div class="flex items-start gap-3">
            <div
              class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white text-[#3082e3] shadow-[0_6px_16px_rgba(48,130,227,0.16)]">
      ...
      <!-- MODAL: ¿Compartir? -->
      <div v-if="viewState === 'confirmShare'"
        ...
        <div class="w-full max-w-sm rounded-[26px] bg-white p-6 shadow-[0_22px_48px_rgba(15,23,42,0.22)]">
          <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#eef4ff] text-[#3082e3]">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
              stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M18 8a3 3 0 10-2.83-4H15a3 3 0 000 6h.17A3 3 0 1018 8zM6 8a3 3 0 100 6 3 3 0 000-6zm0 8c-2.21 0-4 1.79-4 4h8c0-2.21-1.79-4-4-4zm12 0c-.62 0-1.21.14-1.74.38A5.99 5.99 0 0118 20h4c0-2.21-1.79-4-4-4z" />
            </svg>
          </div>
          ...
          <div class="mt-6 space-y-3">
            ...
            <button type="button" @click="continueWithoutSharing"
      ...
      <div v-if="viewState === 'pickContact'" class="fixed inset-0 z-40 bg-black/35"
        @click="viewState = 'confirmShare'"></div>
      <div v-if="viewState === 'pickContact'"
        ...
        <div v-else class="mt-4 mb-6 max-h-72 space-y-2 overflow-y-auto">
          <button v-for="c in trustedContacts" :key="c.id" type="button" @click="chooseContact(c)"
            ...
            <div
              class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#eef4ff] text-[#3082e3] font-bold">
      ...
      <section v-if="viewState === 'preview'" class="px-4 pt-4">
        ...
        <div v-else-if="routeResult"
          class="rounded-[22px] border border-[#e4ebf4] bg-white p-4 shadow-[0_10px_24px_rgba(148,163,184,0.14)]">
          <div class="flex items-center gap-2 flex-wrap">
            <span class="rounded-full bg-[#eef4ff] px-3 py-1 text-[11px] font-semibold text-[#3082e3]">
              Ruta más segura
            </span>
            <span v-if="pendingContact"
          ...
          <button type="button" @click="handleStartRoute"
      ...
      <section v-if="viewState === 'active'" class="px-4 pt-4">
        <div class="rounded-3xl p-5 text-white shadow-[0_14px_30px_rgba(48,130,227,0.28)]"
          style="background: linear-gradient(135deg, #3082e3 0%, #085baf 100%);">
          ...
          <div v-if="activeAlert"
            ...
            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-[#f2826d]">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M12 9v3.75m0 3.75h.007M4.62 19.5h14.76c1.42 0 2.31-1.53 1.61-2.77L13.62 4.66c-.71-1.26-2.53-1.26-3.24 0L3.01 16.73c-.7 1.24.19 2.77 1.61 2.77z" />
              </svg>
            </span>
            <div class="min-w-0">
        ...
        <button type="button" @click="openFinishConfirm"
      ...
      <div v-if="showFinishConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
        <div class="w-full max-w-sm rounded-[26px] bg-white p-6 shadow-[0_22px_48px_rgba(15,23,42,0.22)]">
          ...
          <div class="mt-6 space-y-3">
            <button type="button" @click="confirmFinish(true)"
              class="w-full rounded-2xl bg-[#3082e3] py-3 text-sm font-semibold text-white transition hover:bg-[#085baf] active:scale-[0.98]">
              Sí, llegué bien
            </button>
            <button type="button" @click="confirmFinish(false)"
              class="w-full rounded-2xl bg-[#eef4ff] py-3 text-sm font-semibold text-[#3082e3] transition active:scale-[0.98]">
              Finalizar sin avisar
            </button>
            <button type="button" @click="showFinishConfirm = false"
