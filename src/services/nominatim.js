...
export async function nominatimSearch(
  ...
  url.searchParams.set('dedupe', String(dedupe)); // 0 = NO deduplicar

  const res = await fetch(url, {
    headers,
    referrer: 'ViaSeguraApp/1.0 (Proyecto escolar - Escuela Da Vinci)',
  });
...
export function composeAddress(addr = {}) {
  ...
  const rawCity =
    addr.city || addr.town || addr.village || addr.municipality || addr.state_district || addr.state;
  const ciudad =
    rawCity && /autónoma.*buenos aires|caba|ciudad de buenos aires/i.test(rawCity)
      ? 'CABA'
      : rawCity;
